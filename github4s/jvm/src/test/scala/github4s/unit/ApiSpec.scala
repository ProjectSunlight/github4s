...
class ApiSpec
  ...
  "Issues >> ListComments" should "return the expected comments when a valid issue number is provided" in {
    val response =
      issues.listComments(
        accessToken,
        headerUserAgent,
  ...
  it should "return an error if invalid issue number is provided" in {
    val response =
      issues.listComments(
        accessToken,
        headerUserAgent,
